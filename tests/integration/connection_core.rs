...
async fn test_successful_connection_establishment() {
    ...
    assert!(connection_result.is_ok(), "Connection should succeed to valid server");
    
    let mut connection = connection_result.unwrap();
    
    // Verify connection state
    assert!(connection.is_authenticated(), "Connection should be authenticated after handshake");
    assert!(connection.peer_identity().is_some(), "Peer identity should be available");
...
async fn test_connection_failure_invalid_address() {
    ...
    assert!(connection_result.is_err(), "Connection should fail with invalid address");
    ...
    assert!(
        error_msg.contains("Failed to connect") || error_msg.contains("invalid") || error_msg.contains("resolve"),
...
async fn test_connection_failure_server_not_running() {
    ...
    // Verify connection failed
    assert!(connection_result.is_err(), "Connection should fail when server is not running");
    ...
    assert!(
        error_msg.contains("refused") || error_msg.contains("timeout") || error_msg.contains("Failed to connect"),
        "Error message should indicate server not available: {}", error_msg
    );
}
...
async fn test_peer_identification_after_connection() {
    ...
    assert!(actual_peer_id != "unknown", "Peer ID should not be placeholder value");
...
async fn test_client_identity_usage() {
    ...
    assert!(send_result.is_ok(), "Client should be able to send messages with its identity");
    ...
    assert_eq!(response.get_payload(), "test_identity", "Should receive echo of sent message");
...
async fn test_connection_proper_cleanup() {
    ...
    assert!(!connection.is_closed(), "Connection should not be closed initially");
    ...
    assert!(connection.is_closed(), "Connection should be marked as closed");
    assert!(!connection.is_authenticated(), "Connection should not be authenticated after close");
    
    // Verify peer identity is cleared after close
    assert!(connection.peer_identity().is_none(), "Peer identity should be cleared after close");
...
async fn test_multiple_sequential_connections() {
    ...
    for i in 0..3 {
        ...
        assert!(connection.is_authenticated(), "Each connection should be authenticated");
        assert!(connection.peer_identity().is_some(), "Each connection should have peer identity");
        
        // Test message exchange on each connection
        let test_message = Message::new_ping(i, format!("test_message_{}", i));
        connection.send_message(test_message).await.expect("Should send message");
        
        let (response, _) = timeout(Duration::from_secs(2), connection.receive_message()).await
            .expect("Should receive response within timeout")
            .expect("Should receive valid response");
        
        assert_eq!(response.get_payload(), format!("test_message_{}", i), "Should receive correct echo");
...
async fn test_multiple_client_identities() {
    ...
    for i in 0..3 {
        ...
        let mut connection = client.connect(&server_addr).await
            .expect(&format!("Connection with identity {} should succeed", i + 1));
        ...
        assert_eq!(response.get_payload(), format!("identity_test_{}", i), "Should receive correct echo");
