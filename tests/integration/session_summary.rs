...
async fn test_session_duration_calculated_and_displayed() {
    ...
    assert!(combined_output.contains("Session Summary") || combined_output.contains("session summary"),
           "Should display session summary section. Output: {}", combined_output);

    // Verify session duration is displayed
    assert!(combined_output.contains("Session duration:") || combined_output.contains("duration"),
           "Should display session duration. Output: {}", combined_output);

    // Verify duration shows reasonable time units (ms, s, or µs)
    assert!(combined_output.contains("ms") || combined_output.contains("µs") || 
            combined_output.contains("us") || combined_output.contains("s"),
           "Should display duration with time units. Output: {}", combined_output);

    // Verify session termination message
    assert!(combined_output.contains("Goodbye!") || combined_output.contains("goodbye") ||
            combined_output.contains("exit") || combined_output.contains("terminated"),
           "Should display session termination indication. Output: {}", combined_output);
...
async fn test_performance_statistics_summarized() {
    ...
    assert!(combined_output.contains("Average round-trip time") || 
           ...
           "Should display average performance statistics. Output: {}", combined_output);
    ...
    assert!(combined_output.contains("Messages sent: 4") || 
            (combined_output.contains("4") && combined_output.contains("message")),
           "Should accurately report 4 messages sent. Output: {}", combined_output);
...
async fn test_appropriate_summary_no_messages_sent() {
    ...
    assert!(combined_output.contains("Session Summary"),
           "Should display session summary even with no messages. Output: {}", combined_output);

    // Verify session duration is still calculated and shown
    assert!(combined_output.contains("Session duration:") || combined_output.contains("duration"),
           "Should display session duration even with no messages. Output: {}", combined_output);

    // Verify appropriate message for no messages sent
    assert!(combined_output.contains("No messages sent during this session") || 
            combined_output.contains("0 messages") || combined_output.contains("Messages sent: 0"),
           "Should indicate no messages were sent. Output: {}", combined_output);

    // Verify no performance statistics are shown when no messages were sent
    assert!(!combined_output.contains("Average round-trip time"),
           "Should not show average round-trip time when no messages sent. Output: {}", combined_output);

    // Verify session termination message
    assert!(combined_output.contains("Goodbye!"),
           "Should display session termination message. Output: {}", combined_output);
...
async fn test_clear_session_termination_indication() {
    ...
    assert!(combined_output.contains("Goodbye!"),
           "Should display clear termination message. Output: {}", combined_output);

    // Verify session summary appears before termination
    assert!(combined_output.contains("Session Summary"),
           "Should display session summary before termination. Output: {}", combined_output);
    ...
    assert!(summary_pos.is_some() && goodbye_pos.is_some(),
           "Both summary and goodbye should be present. Output: {}", combined_output);
    
    assert!(summary_pos.unwrap() < goodbye_pos.unwrap(),
           "Session summary should appear before goodbye message. Output: {}", combined_output);

    // Verify program terminates successfully
    assert!(command_output.status.success(),
           "Program should terminate successfully. Exit code: {:?}", command_output.status.code());
...
async fn test_summary_information_well_formatted() {
    ...
    assert!(combined_output.contains("=== Session Summary ===") || 
           ...
           "Should have clear session summary header. Output: {}", combined_output);

    // Verify structured information layout
    assert!(combined_output.contains("Session duration:"),
           "Should have structured session duration field. Output: {}", combined_output);
    
    assert!(combined_output.contains("Messages sent:"),
           "Should have structured messages sent field. Output: {}", combined_output);
    
    assert!(combined_output.contains("Average round-trip time:"),
           "Should have structured performance metrics field. Output: {}", combined_output);
    ...
    assert!(!summary_section.is_empty(),
           "Should have identifiable summary section. Output: {}", combined_output);

    // Verify no extraneous debug information in summary
    assert!(!summary_section.to_lowercase().contains("debug") && 
            !summary_section.to_lowercase().contains("error"),
           "Summary should not contain debug/error information. Summary: {}", summary_section);
...
async fn test_comprehensive_session_summary() {
    ...
    println!("Comprehensive session summary test output:\n{}", combined_output);

    // Comprehensive check of all session summary features
    let checks = vec![
        ("session_summary_header", combined_output.contains("Session Summary")),
        ("session_duration_calculated", combined_output.contains("Session duration:")),
        ("message_count_accurate", combined_output.contains("Messages sent: 4") || 
         (combined_output.contains("4") && combined_output.contains("message"))),
        ("performance_statistics", combined_output.contains("Average round-trip time:")),
        ("time_units_displayed", combined_output.contains("ms") || combined_output.contains("µs") || combined_output.contains("us")),
        ("termination_indication", combined_output.contains("Goodbye!")),
        ("all_messages_processed", combined_output.matches("Received echo").count() == 4),
        ("individual_timings_recorded", combined_output.matches("round-trip").count() >= 4),
        ("proper_formatting", combined_output.contains("===") || combined_output.matches(':').count() >= 3),
    ...
    println!("   - {}/{} session summary features verified", passed_checks, checks.len());
