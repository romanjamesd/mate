...
async fn test_successful_message_send_and_echo_response() {
    ...
    println!("Message send and echo response output:\n{}", combined_output);

    // Verify message was sent
    assert!(combined_output.contains("Sending Ping message"),
           "Should show message being sent. Output: {}", combined_output);

    // Verify echo response was displayed
    assert!(combined_output.contains("Received echo") || combined_output.contains("← Received echo"),
           "Should display echo response. Output: {}", combined_output);

    // Verify the actual message content appears in the echo
    assert!(combined_output.contains("Hello from interactive test"),
           "Echo should contain the actual message content. Output: {}", combined_output);

    // Verify successful exchange
    assert!(command_output.status.success(),
           "Message exchange should complete successfully. Status: {}", command_output.status);
...
async fn test_message_statistics_tracked() {
    ...
    assert!(combined_output.contains("Messages sent: 4") || combined_output.contains("4") && combined_output.contains("message"),
           "Should track message count accurately. Output: {}", combined_output);

    // Verify session summary shows statistics
    assert!(combined_output.contains("Session Summary") || combined_output.contains("session"),
           "Should show session summary with statistics. Output: {}", combined_output);

    // Verify all messages were processed
    let echo_count = combined_output.matches("Received echo").count();
    assert_eq!(echo_count, 4,
...
async fn test_performance_metrics_accumulate() {
    ...
    println!("Performance metrics accumulation output:\n{}", combined_output);

    // Verify average timing is calculated
    assert!(combined_output.contains("Average") || combined_output.contains("average"),
           "Should show average performance metrics. Output: {}", combined_output);
    ...
    assert!(combined_output.contains("round-trip") || combined_output.contains("performance"),
           "Should include performance data in session summary. Output: {}", combined_output);

    // Verify individual message timings were recorded
    let timing_mentions = combined_output.matches("round-trip").count();
    assert!(timing_mentions >= 3,
...
async fn test_clear_indication_of_received_responses() {
    ...
    assert!(combined_output.contains("Received echo") || combined_output.contains("← Received"),
           "Should clearly indicate received responses. Output: {}", combined_output);

    // Verify each test message was clearly echoed
    for message in &test_messages {
        assert!(combined_output.contains(message),
               "Message '{}' should be clearly indicated in response. Output: {}", message, combined_output);
    }
    ...
    assert!(timing_with_responses >= test_messages.len(),
           "Should include timing with response indications. Expected: {}, Got: {}, Output: {}", 
           test_messages.len(), timing_with_responses, combined_output);
...
async fn test_multiple_exchanges_maintain_accurate_statistics() {
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        ...
        for i in 1..=3 {
            let _ = stdin.write_all(format!("Batch 1 Message {}\n", i).as_bytes()).await;
            tokio::time::sleep(Duration::from_millis(300)).await;
        }
        ...
        for i in 1..=4 {
            let _ = stdin.write_all(format!("Batch 2 Message {}\n", i).as_bytes()).await;
            tokio::time::sleep(Duration::from_millis(300)).await;
        }
...
async fn test_comprehensive_interactive_message_exchange() {
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        ...
        let _ = stdin.write_all(b"This is a longer message to test different response characteristics\n").await;
        tokio::time::sleep(Duration::from_millis(400)).await;
    ...
    println!("Comprehensive message exchange output:\n{}", combined_output);

    // Verify all message exchange features worked
    let checks = vec![
        ("messages_sent", combined_output.matches("Sending Ping message").count() >= 4), // 4 user messages + handshake
        ("responses_received", combined_output.matches("Received echo").count() == 4), // 4 user messages
        ("timing_measured", combined_output.matches("round-trip").count() >= 4),
        ("statistics_tracked", combined_output.contains("Messages sent: 4") || (combined_output.contains("4") && combined_output.contains("message"))),
        ("performance_metrics", combined_output.contains("Average") || combined_output.contains("average")),
        ("clear_responses", combined_output.contains("Initial test message") && combined_output.contains("longer message")),
    ...
    println!("   - {}/{} message exchange features verified", passed_checks, checks.len());
