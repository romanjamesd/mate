...
async fn test_connection_information_display() {
    ...
    assert!(
        combined_output.contains("Connected") || combined_output.contains("connection"),
        "Output should display connection information. Output: {}",
        combined_output
    );

    // Verify server address information is shown
    assert!(
        combined_output.contains(server_addr) || combined_output.contains("127.0.0.1"),
        "Output should display server address information. Output: {}",
        combined_output
    );
...
async fn test_connection_status_communication() {
    ...
    assert!(
        ...
        "Output should clearly communicate connection status. Output: {}",
        combined_output
    );
    ...
    assert!(
        ...
        "Output should not indicate connection failure. Output: {}",
        combined_output
    );
...
async fn test_available_functionality_explanation() {
    ...
    assert!(
        combined_output.contains("help")
            ...
            || combined_output.contains("functionality"),
        "Output should explain available functionality. Output: {}",
        combined_output
    );
    ...
    assert!(
        has_command_info,
        "Output should mention available commands. Output: {}",
        combined_output
    );
...
async fn test_usage_instructions_provided() {
    ...
    assert!(
        combined_output.contains("Type")
            ...
            || combined_output.contains("instructions")
            || combined_output.contains("how to"),
        "Output should provide usage instructions. Output: {}",
        combined_output
    );
    ...
    assert!(
        combined_output.contains("message")
            || combined_output.contains("send")
            || combined_output.contains("type")
            || combined_output.contains("enter"),
        "Output should explain how to send messages. Output: {}",
        combined_output
    );
...
async fn test_visual_separation_between_sections() {
    ...
    assert!(
        has_separation,
        "Output should have visual separation between sections. Output: {}",
        combined_output
    );
    ...
    assert!(
        line_count > 1,
        "Output should have multiple lines for structure. Output: {}",
        combined_output
    );
...
async fn test_session_tracking_initialization() {
    ...
    assert!(
        combined_output.contains("session")
            ...
            || combined_output.contains("started"),
        "Output should show session tracking initialization. Output: {}",
        combined_output
    );
    ...
    assert!(
        ...
        "Output should show connection tracking. Output: {}",
        combined_output
    );
