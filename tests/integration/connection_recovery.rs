...
async fn test_reconnection_behavior_when_sending_fails() {
    ...
    println!("Reconnection when sending fails output:\n{}", combined_output);

    // Verify initial message was sent successfully
    assert!(combined_output.contains("Initial message before failure"),
           "Initial message should be processed. Output: {}", combined_output);

    // Verify connection issues are detected and communicated
    assert!(combined_output.contains("connection") || combined_output.contains("error") || 
            combined_output.contains("failed") || combined_output.contains("retry"),
           "Should indicate connection issues or retry attempts. Output: {}", combined_output);
    ...
    assert!(command_output.status.success(),
           "Client should handle connection failure gracefully. Status: {}", command_output.status);
...
async fn test_reconnection_behavior_when_receiving_fails() {
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        ...
        let _ = stdin.write_all(b"Message after receive failure recovery\n").await;
        tokio::time::sleep(Duration::from_millis(500)).await;
    ...
    println!("Reconnection when receiving fails output:\n{}", combined_output);

    // Verify initial message was processed
    assert!(combined_output.contains("Message before receiving failure"),
           "Initial message should be in output. Output: {}", combined_output);

    // Verify recovery message was processed after reconnection
    assert!(combined_output.contains("Message after receive failure recovery"),
           "Recovery message should be processed. Output: {}", combined_output);

    // Verify connection recovery handling
    assert!(combined_output.contains("connection") || combined_output.contains("error") || 
            combined_output.contains("failed") || combined_output.contains("retry"),
           "Should indicate connection recovery attempts. Output: {}", combined_output);

    // Verify graceful handling
    assert!(command_output.status.success(),
           "Client should handle receive failure gracefully. Status: {}", command_output.status);
...
async fn test_connection_status_changes_communicated() {
    ...
    println!("Connection status changes communication output:\n{}", combined_output);
    ...
    assert!(combined_output.contains("Connected") || combined_output.contains("connection") || 
           ...
           "Should show initial connection status. Output: {}", combined_output);

    // Verify connection issues are communicated
    assert!(combined_output.contains("error") || combined_output.contains("failed") || 
            combined_output.contains("disconnect") || combined_output.contains("connection"),
           "Should communicate connection status changes. Output: {}", combined_output);
...
async fn test_reconnection_preserves_session_state() {
    ...
    assert!(combined_output.contains("Session duration") || combined_output.contains("duration"),
           "Should show session duration preservation. Output: {}", combined_output);
    ...
    assert!(combined_output.contains("session") || combined_output.contains("duration") ||
           ...
           "Should preserve session state information. Output: {}", combined_output);
...
async fn test_multiple_reconnection_failures_handled() {
    ...
    println!("Multiple reconnection failures output:\n{}", combined_output);

    // Verify initial message was processed
    assert!(combined_output.contains("Initial message"),
           "Initial message should be processed. Output: {}", combined_output);
    ...
    assert!(connection_issues >= 2,
           "Should handle multiple connection issues. Connection issues: {}, Output: {}", 
           connection_issues, combined_output);

    // Verify client doesn't crash despite multiple failures
    assert!(command_output.status.success(),
           "Client should survive multiple connection failures. Status: {}", command_output.status);
...
async fn test_comprehensive_connection_recovery() {
    ...
    println!("Comprehensive connection recovery output:\n{}", combined_output);

    // Verify all connection recovery features worked
    let checks = vec![
        ("normal_operation", combined_output.contains("Normal operation message")),
        ("connection_status", combined_output.contains("connection") || combined_output.contains("status")),
        ("failure_detection", combined_output.contains("error") || combined_output.contains("failed") || combined_output.contains("connection")),
        ("recovery_attempts", combined_output.contains("Attempt 1") || combined_output.contains("Attempt 2")),
        ("session_preservation", combined_output.contains("session") || combined_output.contains("duration")),
    ...
    println!("   - {}/{} connection recovery features verified", passed_checks, checks.len());
