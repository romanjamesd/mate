...
async fn test_server_bind_and_basic_startup() {
    ...
    assert!(server_result.is_ok(), "Server should not return errors during clean shutdown");
}
...
async fn test_server_multiple_bind_attempts() {
    ...
    assert!(result.is_err(), "Second server should fail to bind to the same address");
