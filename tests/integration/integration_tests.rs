...
async fn test_complete_workflow_connection_to_termination() {
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        ...
        let _ = stdin.write_all(b"Hello, testing complete workflow!\n").await;
    ...
    let workflow_phases = vec![
        ("connection_establishment", combined_output.contains("Connected")),
        ("help_functionality", combined_output.contains("Available commands") || combined_output.contains("help")),
        ("info_functionality", combined_output.contains("Peer ID") || combined_output.contains("Connection status")),
        ("message_exchange", combined_output.contains("Hello, testing complete workflow!")),
        ("sustained_messaging", combined_output.contains("Workflow message 3")),
        ("statistics_tracking", combined_output.contains("Messages sent")),
        ("timing_calculation", combined_output.contains("round-trip") || combined_output.contains("Average")),
        ("session_summary", combined_output.contains("Session Summary") || combined_output.contains("Session duration")),
    ...
    assert!(successful_phases >= 8,
           "At least 8/9 workflow phases should succeed. Succeeded: {}/9. Output: {}", 
           successful_phases, combined_output);

    // Verify message count accuracy
    assert!(combined_output.contains("Messages sent: 5") || combined_output.contains("Messages sent: 6"),
           "Should accurately track all messages sent during workflow. Output: {}", combined_output);

    // Verify session duration is reasonable
    assert!(session_duration < Duration::from_secs(12),
           "Complete workflow should complete in reasonable time. Took: {:?}", session_duration);

    println!("✅ Complete workflow from connection establishment to termination test passed");
    println!("   - {}/{} workflow phases completed successfully", successful_phases, workflow_phases.len());
...
async fn test_combinations_commands_messages_single_session() {
    ...
    println!("Command/message combinations test output:\n{}", combined_output);
    ...
    assert!(help_count >= 3,
           "Help commands should be processed multiple times. Found: {} instances", help_count);

    let info_count = combined_output.lines()
        .filter(|line| line.contains("Messages sent") || line.contains("Session duration"))
        .count();
    assert!(info_count >= 4,
           "Info commands should be processed multiple times. Found: {} instances", info_count);

    // Verify message count tracking remains accurate despite command interspersion
    assert!(combined_output.contains("Messages sent: 9") || combined_output.contains("Messages sent: 8"),
           "Should accurately count messages despite command interspersion. Output: {}", combined_output);

    // Verify commands don't interfere with message timing
    assert!(combined_output.contains("Average round-trip time") || combined_output.contains("round-trip"),
           "Message timing should work despite command interspersion. Output: {}", combined_output);

    // Verify session state coherence
    assert!(combined_output.contains("Session Summary") || combined_output.contains("Goodbye"),
           "Session should maintain coherent state throughout combinations. Output: {}", combined_output);
...
async fn test_reconnection_followed_by_continued_operation() {
    ...
    println!("Reconnection and continued operation test output:\n{}", combined_output);

    // Verify pre-reconnection operation
    assert!(combined_output.contains("Pre-reconnection message 1"),
           "Pre-reconnection messages should be processed. Output: {}", combined_output);
    ...
    assert!(disruption_detected,
           "Connection disruption should be detected. Output: {}", combined_output);
    ...
    assert!(post_reconnection_success,
           "Post-reconnection operation should succeed. Output: {}", combined_output);
    ...
    assert!(commands_work_after_reconnection,
           "Commands should work after reconnection. Output: {}", combined_output);
    ...
    assert!(session_coherent,
           "Session should maintain coherence after reconnection. Output: {}", combined_output);
    ...
    assert!(stability_confirmed,
           "System should be stable after reconnection. Output: {}", combined_output);
...
async fn test_appropriate_information_logged_throughout_session() {
    ...
    let logging_categories = vec![
        ("connection_info", combined_output.contains("Connected to peer") || combined_output.contains("Peer ID")),
        ("session_tracking", combined_output.contains("Session duration") || combined_output.contains("session")),
        ("message_statistics", combined_output.contains("Messages sent") || combined_output.contains("message")),
        ("timing_information", combined_output.contains("round-trip") || combined_output.contains("Average")),
        ("command_responses", combined_output.contains("Available commands") || combined_output.contains("help")),
        ("status_updates", combined_output.contains("Connection status") || combined_output.contains("status")),
        ("session_summary", combined_output.contains("Session Summary") || combined_output.contains("summary")),
    ];
    ...
    assert!(logged_categories >= 6,
           "At least 6/7 information categories should be logged. Found: {}/7. Output: {}", 
           logged_categories, combined_output);

    // Verify appropriate detail level (not too verbose, not too sparse)
    let total_lines = combined_output.lines().count();
    assert!(total_lines >= 20 && total_lines <= 200,
           "Logging should have appropriate detail level. Found: {} lines", total_lines);

    // Verify critical events are logged
    assert!(combined_output.contains("Logging test message 1"),
           "Message content should be logged appropriately. Output: {}", combined_output);

    // Verify session lifecycle is tracked
    let lifecycle_events = vec!["connected", "session", "duration", "summary", "goodbye"];
    let lifecycle_found = lifecycle_events.iter()
        .filter(|event| combined_output.to_lowercase().contains(&event.to_lowercase()))
        .count();

    assert!(lifecycle_found >= 3,
           "Session lifecycle should be logged. Found: {}/5 lifecycle events", lifecycle_found);

    // Verify no excessive error spam
    let error_lines = combined_output.lines()
        .filter(|line| line.to_lowercase().contains("error"))
        .count();
    
    assert!(error_lines <= 5,
           "Should not have excessive error logging. Found: {} error lines", error_lines);
    ...
    assert!(structured_info,
           "Should include structured logging information. Output: {}", combined_output);

    println!("✅ Appropriate information logged throughout session test passed");
    println!("   - {}/7 information categories properly logged", logged_categories);
    println!("   - {}/5 session lifecycle events found", lifecycle_found);
    println!("   - Total output: {} lines (appropriate detail level)", total_lines);
...
async fn test_behavior_consistency_across_terminal_environments() {
    ...
    let environment_configs = vec![
        ...
        (vec![("COLUMNS", "80"), ("LINES", "24")], "standard terminal size"),
        (vec![("COLUMNS", "120"), ("LINES", "40")], "larger terminal size"),
    ];
    ...
    for (env_vars, description) in environment_configs {
        ...
        let command_output = output
            .expect(&format!("Command should complete within timeout for {}", description))
            .expect(&format!("Command should execute successfully for {}", description));
        ...
        let environment_analysis = EnvironmentTestResult {
            ...
            contains_help: combined_output.contains("Available commands") || combined_output.contains("help"),
            contains_message: combined_output.contains("Environment test message"),
            contains_info: combined_output.contains("Messages sent") || combined_output.contains("Session duration"),
            contains_summary: combined_output.contains("Session Summary") || combined_output.contains("Goodbye"),
    ...
    assert!(consistent_exit_codes,
           "Exit codes should be consistent across environments. Found: {:?}", exit_codes);
    ...
    let output_lengths: Vec<usize> = environment_results.iter().map(|r| r.output_length).collect();
    ...
    assert!(length_variation_ratio < 2.0,
           "Output length should be reasonably consistent. Variation ratio: {:.2}, lengths: {:?}", 
           length_variation_ratio, output_lengths);
    println!("   ✓ Output length reasonably consistent (variation ratio: {:.2})", length_variation_ratio);

    // Check that no environment has excessive errors
    let environments_with_errors = environment_results.iter()
        .filter(|r| r.has_errors)
        .count();
    
    assert!(environments_with_errors <= 1,
...
async fn test_comprehensive_integration_functionality() {
    ...
    println!("Comprehensive integration test output:\n{}", combined_output);

    // Comprehensive integration checks
    let integration_checks = vec![
        ("connection_establishment", combined_output.contains("Connected")),
        ("help_command_functionality", combined_output.contains("Available commands") || combined_output.contains("help")),
        ("info_command_functionality", combined_output.contains("Messages sent") && combined_output.contains("Session duration")),
        ("basic_message_processing", combined_output.contains("Integration test message 1")),
        ("unicode_message_support", combined_output.contains("测试") || combined_output.contains("🚀")),
        ("varied_content_handling", combined_output.contains("Longer integration message")),
        ("rapid_sequence_processing", combined_output.contains("Rapid integration 3")),
        ("statistics_calculation", combined_output.contains("Average round-trip time") || combined_output.contains("round-trip")),
        ("session_state_tracking", combined_output.contains("Session duration")),
        ("message_count_accuracy", combined_output.contains("Messages sent: 8") || combined_output.contains("Messages sent: 7")),
        ("graceful_termination", command_output.status.success()),
    ...
    assert!(passed_integration_checks >= 10,
           "At least 10/12 integration checks should pass. Passed: {}/12. Output: {}", 
           passed_integration_checks, combined_output);

    // Verify performance is reasonable
    assert!(integration_duration < Duration::from_secs(12),
           "Comprehensive integration should complete in reasonable time. Took: {:?}", integration_duration);

    // Verify no crashes or hangs
    assert!(command_output.status.success(),
           "Integration test should complete successfully. Status: {}", command_output.status);

    println!("✅ Comprehensive integration functionality test passed");
    println!("   - {}/{} integration features verified", passed_integration_checks, integration_checks.len());
