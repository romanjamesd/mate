...
async fn test_corrupted_length_prefix_handling() {
    ...
    println!("Test 4: Testing protocol recovery after invalid length prefix");
    {
        ...
        framed_message.write_message(&mut valid_stream, &valid_envelope)
            .await
        ...
        let recovered_envelope = framed_message.read_message(&mut read_stream)
            .await
...
async fn test_corrupted_message_data_handling() {
    ...
    println!("Test 4: Testing protocol recovery after corrupted data");
    {
        ...
        framed_message.write_message(&mut recovery_write_stream, &recovery_envelope)
            .await
        ...
        let recovered_envelope = framed_message.read_message(&mut recovery_read_stream)
            .await
    ...
    println!("Test 1: Testing connection closure during length prefix read");
    {
        ...
        match result {
            Err(e) => {
                ...
                assert!(!e.to_string().is_empty(), "Error should have a meaningful message");
            },
            Ok(_) => {
    ...
    println!("Test 5: Testing resource cleanup during connection closure scenarios");
    {
        ...
        for (i, scenario_data) in scenarios.iter().enumerate() {
            ...
            match result {
                ...
                Ok(_) => {
                    panic!("Scenario {} unexpectedly succeeded with {} bytes of data", 
                           i, scenario_data.len());
                }
            }
        }
...
async fn test_protocol_violation_detection() {
    ...
    println!("Test 3: Testing protocol compliance edge cases");
    {
        ...
        match read_result {
            Ok(envelope) => {
                assert!(envelope.verify_signature(), "Valid message should have valid signature");
        ...
        if valid_data.len() > 8 {
            ...
            let original_length = u32::from_be_bytes([
                modified_data[0], modified_data[1], modified_data[2], modified_data[3]
            ]);
            ...
            if original_length > 1 {
                ...
                match modified_result {
                    ...
                    Ok(_) => {
                        println!("  ⚠ Modified length prefix was accepted (may be within tolerance)");
                    }
                }
            }
        }
    ...
    println!("Test 4: Testing multiple consecutive protocol violations");
    {
        ...
        framed_message.write_message(&mut recovery_stream, &recovery_envelope)
            .await
            .expect("Recovery message should write successfully");
        ...
        let final_envelope = framed_message.read_message(&mut recovery_read_stream)
            .await
            .expect("Recovery message should read successfully after violations");
        
        assert!(final_envelope.verify_signature(), 
