...
async fn test_large_message_handling() {
    ...
    for (payload_size, description) in large_message_test_cases {
        ...
        if serialized_size <= large_message_limit {
            ...
            let write_result = framed_message.write_message(&mut large_stream, &large_envelope).await;
            match write_result {
                Ok(_) => {
                    println!("    ✓ Large message ({} bytes) written successfully", serialized_size);
            ...
            println!("    ! Large message ({} bytes) exceeds limit ({} bytes) due to envelope overhead", 
