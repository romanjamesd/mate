...
impl TestEnvironment {
    fn new() -> (Database, Self) {
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .as_nanos();
        let unique_temp_dir = temp_dir.path().join(format!("test_integration_{}", timestamp));
...
fn test_complete_game_lifecycle_with_messages() {
    ...
    let _game_accept_msg = db.store_message(
        game.id.clone(),
...
fn test_concurrent_games_workflow() {
    ...
    let opponent1_games = db.get_games_with_opponent("opponent_concurrent_1")
        .expect("Failed to get games with opponent 1");
    assert_eq!(opponent1_games.len(), 2, "Should have 2 games with opponent 1");
    
    let completed_games = db.get_games_by_status(GameStatus::Completed)
        .expect("Failed to get completed games");
    assert_eq!(completed_games.len(), 1, "Should have 1 completed game");
    
    let abandoned_games = db.get_games_by_status(GameStatus::Abandoned)
...
fn test_game_with_extensive_message_history() {
    ...
    for i in 0..50 {
        let sender = if i % 2 == 0 { "test_peer_integration" } else { "opponent_extensive" };
    ...
    assert_eq!(all_messages.len(), expected_total, "Should have all messages stored");
    
    // Test pagination on large message set
    let page1 = db.get_messages_for_game_paginated(&game.id, 10, 0)
    ...
    let last_page = db.get_messages_for_game_paginated(&game.id, 10, last_page_offset as u32)
        .expect("Failed to get last page");
    assert!(last_page.len() <= 10, "Last page should have <= 10 messages");
    
    // Verify message type distribution
    let move_messages = db.get_messages_by_type(&game.id, "move")
        .expect("Failed to get move messages");
    assert_eq!(move_messages.len(), move_count, "Should have correct number of move messages");
    
    let chat_messages = db.get_messages_by_type(&game.id, "chat")
        .expect("Failed to get chat messages");
    assert_eq!(chat_messages.len(), chat_count, "Should have correct number of chat messages");
    
    // Test message count function
    let total_count = db.count_messages_for_game(&game.id)
        .expect("Failed to count messages");
    assert_eq!(total_count as usize, expected_total, "Message count should match actual count");
}
...
fn test_database_connection_management() {
    ...
    for i in 0..20 {
        let game = db.create_game(
            ...
            if i % 2 == 0 { PlayerColor::White } else { PlayerColor::Black },
            None,
    ...
    assert!(final_stats.0 > initial_stats.0, "Operations count should increase");
    assert_eq!(final_stats.2, initial_stats.2, "Error count should remain the same");
    
    // Test connection health
    assert!(db.check_connection_health().unwrap(), "Connection should remain healthy");
    
    // Test maintenance operations
    db.perform_maintenance().expect("Maintenance should succeed");
    
    // Verify database is still functional after maintenance
    let test_game = db.create_game(
        "post_maintenance_test".to_string(),
        PlayerColor::White,
        None,
    ).expect("Should be able to create game after maintenance");
    
    assert!(db.get_game(&test_game.id).is_ok(), "Should be able to retrieve game after maintenance");
}
...
fn test_transaction_consistency() {
    ...
    assert!(completed_at >= updated_at - 1000, "Timestamps should be close"); // Allow 1 second tolerance
}
...
fn test_database_schema_creation() {
    ...
    let retrieved_game = db.get_game(&game.id)
        .expect("Game retrieval should work");
    assert!(retrieved_game.metadata.is_some(), "Metadata should be preserved");
    ...
    assert_eq!(retrieved_message.game_id, game.id, "Foreign key relationship should work");
    ...
    assert!(message_result.is_err(), "Message should be deleted when game is deleted");
}
...
fn test_database_indexes_and_performance() {
    ...
    let opponent_0_games = db.get_games_with_opponent("opponent_0")
        .expect("Failed to query games by opponent");
    assert!(opponent_0_games.len() > 0, "Should find games for opponent_0");
    ...
    for game_id in &game_ids {
        let messages = db.get_messages_for_game(game_id)
    ...
    let move_messages = db.get_messages_by_type(&game_ids[0], "move")
    ...
    let recent_games = db.get_recent_games(5)
    ...
    assert_eq!(recent_messages.len(), 10, "Should get 10 most recent messages");
}
...
fn test_data_integrity_and_consistency() {
    ...
    for i in 1..message_times.len() {
        assert!(message_times[i] >= message_times[i-1], 
    ...
    let updated_game = db.get_game(&game.id)
        .expect("Failed to get updated game");
    
    assert!(updated_game.updated_at > pre_update_time, 
            "Updated timestamp should increase after modification");
    assert_eq!(updated_game.created_at, initial_time, 
