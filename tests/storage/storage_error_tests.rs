...
fn test_database_path_errors() {
    ...
    assert!(result.is_ok() || result.is_err(), "Database creation should not panic");
...
fn test_corrupted_database_handling() {
    ...
    fs::write(&db_path, "This is not a valid SQLite database file").expect("Failed to write corrupted file");
    ...
    if let Err(error) = result {
        ...
        match error {
            ...
            other => panic!("Expected ConnectionFailed or MigrationFailed error, got: {:?}", other),
        }
    }
}
...
fn test_database_permission_errors() {
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = fs::metadata(&readonly_path).expect("Failed to get metadata").permissions();
...
fn test_game_not_found_errors() {
    ...
    assert!(result.is_err(), "Updating nonexistent game status should fail");
    ...
    assert!(result.is_err(), "Updating nonexistent game result should fail");
...
fn test_invalid_game_data_errors() {
    ...
    assert!(result.is_ok() || result.is_err(), "Invalid metadata handling should not panic");
    ...
    assert!(result.is_ok() || result.is_err(), "Long peer ID handling should not panic");
}
...
fn test_concurrent_game_modification_errors() {
    ...
    assert!(result_result.is_err(), "Setting result for deleted game should fail");
...
fn test_message_not_found_errors() {
    ...
    match result.unwrap_err() {
        StorageError::MessageNotFound { id } => {
            assert_eq!(id, nonexistent_message_id.to_string(), "Error should include the message ID");
        }
    ...
    assert!(result.is_err(), "Deleting nonexistent message should return an error");
}
...
fn test_message_with_invalid_game_id() {
    ...
    match result {
        Ok(message) => {
            ...
            assert!(retrieved.is_ok(), "Should be able to retrieve stored message");
        }
        Err(_) => {
...
fn test_message_data_validation_errors() {
    ...
    assert!(result.is_ok() || result.is_err(), "Long content should be handled gracefully");
    ...
    assert!(result.is_ok() || result.is_err(), "Empty strings should be handled gracefully");
    ...
    assert!(result.is_ok() || result.is_err(), "Null characters should be handled gracefully");
}
...
fn test_invalid_query_parameters() {
    ...
    let game = db.create_game("pagination_test".to_string(), PlayerColor::White, None)
        .expect("Failed to create game");
    
    // Test with very large offset
    let result = db.get_messages_for_game_paginated(&game.id, 10, u32::MAX);
    assert!(result.is_ok(), "Large offset should return empty result, not error");
    
    let messages = result.unwrap();
    assert_eq!(messages.len(), 0, "Should return empty result for offset beyond data");
    
    // Test with zero limit
    let result = db.get_messages_for_game_paginated(&game.id, 0, 0);
    assert!(result.is_ok(), "Zero limit should return empty result, not error");
    
    let messages = result.unwrap();
    assert_eq!(messages.len(), 0, "Should return empty result for zero limit");
...
fn test_query_with_invalid_references() {
    ...
    let messages = db.get_messages_for_game(nonexistent_game_id)
        .expect("Should return empty result for nonexistent game");
    assert_eq!(messages.len(), 0, "Should return empty result for nonexistent game");
    
    let paginated_messages = db.get_messages_for_game_paginated(nonexistent_game_id, 10, 0)
        .expect("Should return empty result for nonexistent game");
    assert_eq!(paginated_messages.len(), 0, "Should return empty result for nonexistent game");
    
    let typed_messages = db.get_messages_by_type(nonexistent_game_id, "move")
        .expect("Should return empty result for nonexistent game");
    assert_eq!(typed_messages.len(), 0, "Should return empty result for nonexistent game");
    
    let sender_messages = db.get_messages_from_sender(nonexistent_game_id, "some_sender")
        .expect("Should return empty result for nonexistent game");
    assert_eq!(sender_messages.len(), 0, "Should return empty result for nonexistent game");
    
    let message_count = db.count_messages_for_game(nonexistent_game_id)
        .expect("Should return 0 for nonexistent game");
    assert_eq!(message_count, 0, "Should return 0 count for nonexistent game");
    
    let opponent_games = db.get_games_with_opponent(nonexistent_peer_id)
        .expect("Should return empty result for nonexistent opponent");
    assert_eq!(opponent_games.len(), 0, "Should return empty result for nonexistent opponent");
    
    // Test deletion of messages for nonexistent game
    let deleted_count = db.delete_messages_for_game(nonexistent_game_id)
        .expect("Should return 0 for nonexistent game");
    assert_eq!(deleted_count, 0, "Should return 0 deleted count for nonexistent game");
}
...
fn test_concurrent_query_and_modification() {
    ...
    let game = db.create_game("concurrent_query_test".to_string(), PlayerColor::White, None)
        .expect("Failed to create game");
    ...
    let initial_count = db.count_messages_for_game(&game.id)
    ...
    let messages = db.get_messages_for_game(&game.id)
        .expect("Should return empty result after game deletion");
    assert_eq!(messages.len(), 0, "Should have no messages after game deletion");
    
    let final_count = db.count_messages_for_game(&game.id)
...
fn test_enum_parsing_errors() {
    ...
    assert_eq!(GameStatus::from_str("completed"), Some(GameStatus::Completed));
    assert_eq!(GameStatus::from_str("abandoned"), Some(GameStatus::Abandoned));
    ...
    assert_eq!(GameResult::from_str("abandoned"), Some(GameResult::Abandoned));
...
fn test_json_serialization_errors() {
    use mate::storage::models::{Game, Message, GameResult};
    ...
    assert!(deserialization_result.is_ok(), "Game deserialization should succeed");
    ...
    assert!(message_json_result.is_ok(), "Message serialization should succeed");
    
    let message_json_str = message_json_result.unwrap();
    let message_deserialization_result: Result<Message, _> = serde_json::from_str(&message_json_str);
    assert!(message_deserialization_result.is_ok(), "Message deserialization should succeed");
    ...
    assert!(invalid_result.is_err(), "Invalid JSON should fail to deserialize");
} 