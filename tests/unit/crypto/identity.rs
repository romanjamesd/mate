...
fn test_identity_generation_and_uniqueness() {
    ...
    assert_ne!(identity1.peer_id(), identity2.peer_id(), "Generated identities should be unique");
    ...
    for i in 0..10 {
        ...
        assert!(peer_ids.insert(peer_id), "Generated identity {} should be unique", i);
    }
}
...
fn test_peer_id_generation_and_serialization() {
    ...
    assert_eq!(&peer_id1, peer_id2, "PeerId generation should be consistent");
    ...
    assert_eq!(peer_id1, peer_id_reconstructed, "String serialization should be round-trip compatible");
    
    // Test verifying key reconstruction
    let reconstructed_key = peer_id_reconstructed.to_verifying_key()
        .expect("Should be able to reconstruct verifying key");
    assert_eq!(verifying_key.to_bytes(), reconstructed_key.to_bytes(), 
    ...
    let peer_id_from_json: PeerId = serde_json::from_str(&json_str).expect("Failed to deserialize from JSON");
    assert_eq!(peer_id1, peer_id_from_json, "JSON serialization should be round-trip compatible");
}
...
fn test_signature_verification_valid() {
    ...
    assert!(!is_cross_valid, "Signature should not verify with different key");
}
...
fn test_signature_verification_invalid_cases() {
    ...
    assert!(!Identity::verify(&wrong_key, message, &signature), 
            "Signature should fail verification with wrong key");
    
    // Test with tampered message
    let tampered_message = b"tampered message";
    assert!(!Identity::verify(&verifying_key, tampered_message, &signature), 
    ...
    assert!(!Identity::verify(&verifying_key, message, &tampered_signature), 
...
fn test_peer_id_equality_and_hashing() {
    ...
    assert_eq!(peer_id1a, peer_id1b, "Same identity should produce equal PeerIds");
    assert_ne!(peer_id1a, peer_id2, "Different identities should produce different PeerIds");
    ...
    assert_eq!(peer_map.get(&peer_id1a), Some(&"peer1".to_string()), "HashMap lookup should work");
    assert_eq!(peer_map.get(&peer_id1b), Some(&"peer1".to_string()), "Equal PeerIds should hash to same value");
    assert_eq!(peer_map.get(&peer_id2), Some(&"peer2".to_string()), "Different PeerIds should be distinct");
    assert_eq!(peer_map.len(), 2, "HashMap should contain exactly 2 entries");
}
...
fn test_peer_id_invalid_input_handling() {
    ...
    assert!(invalid_base64.to_verifying_key().is_err(), 
    ...
    assert!(short_peer_id.to_verifying_key().is_err(), 
    ...
    assert!(long_peer_id.to_verifying_key().is_err(), 
    ...
    assert!(empty_peer_id.to_verifying_key().is_err(), 
...
fn test_signing_edge_cases() {
    ...
    let verifying_key = identity.verifying_key();
    assert!(Identity::verify(&verifying_key, empty_message, &empty_signature), 
    ...
    assert!(Identity::verify(&verifying_key, &large_message, &large_signature), 
    ...
    assert!(Identity::verify(&verifying_key, single_byte, &single_signature), 
...
fn test_verification_edge_cases() {
    ...
    assert!(!Identity::verify(&verifying_key, b"test", &zero_signature), 
    ...
    assert!(!Identity::verify(&verifying_key, b"test", &ones_signature), 
    ...
    if let Ok(corrupted_key) = VerifyingKey::from_bytes(&corrupted_key_bytes) {
        ...
        assert!(!Identity::verify(&corrupted_key, b"test", &test_signature), 
...
fn test_peer_id_display_formatting() {
    ...
    assert_eq!(peer_id, &peer_id_from_str, "Display string should be reversible");
    
    // Verify the display string is valid base64
    assert!(base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &display_str).is_ok(), 
    ...
    assert_eq!(format!("{}", same_peer_id), display_str, 
...
fn test_identity_serialization_errors() {
    ...
    let corrupted_base64 = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &short_bytes);
    ...
    assert!(error_msg.contains("Invalid PeerId key length") || error_msg.contains("32 bytes"), 
    ...
    assert!(result2.is_err(), "Invalid base64 format should produce error");
    
    let error_msg2 = result2.unwrap_err().to_string();
    assert!(error_msg2.contains("Failed to decode PeerId base64") || error_msg2.contains("decode"), 
...
fn test_crypto_memory_safety() {
    ...
    let signature = {
        ...
        assert!(Identity::verify(&verifying_key, message, &sig), 
    ...
    assert!(Identity::verify(&key, message, &sig), 
    ...
    let reconstructed_key = peer_id.to_verifying_key()
        .expect("Should be able to reconstruct key from PeerId");
    
    assert_eq!(key.to_bytes(), reconstructed_key.to_bytes(), 
