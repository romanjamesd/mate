...
mod tests {
    ...
    fn test_precision_adjustment() {
        ...
        assert!(
            result.ends_with("μs"),
            "Sub-millisecond times should use microsecond precision"
        );
        ...
        assert!(
            result.ends_with("ms"),
            "Sub-second times should use millisecond precision"
        );
        ...
        assert!(
            result.ends_with("s"),
            "Multi-second times should use second precision"
        );
    ...
    fn test_response_speed_categories() {
        ...
        for (duration, expected) in very_fast_cases {
            ...
            assert_eq!(
                result, expected,
                "Very fast response formatting failed for {:?}",
                duration
            );
        }
        ...
        for (duration, expected) in typical_cases {
            ...
            assert_eq!(
                result, expected,
                "Typical response formatting failed for {:?}",
                duration
            );
        }
        ...
        for (duration, expected) in slow_cases {
            ...
            assert_eq!(
                result, expected,
                "Slow response formatting failed for {:?}",
                duration
            );
        }
    ...
    fn test_consistency_across_operations() {
        ...
        for (duration, expected) in boundary_cases {
            ...
            for _ in 0..5 {
                ...
                assert_eq!(
                    result, expected,
                    "Inconsistent formatting for boundary case {:?}",
                    duration
                );
            }
        }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            format_round_trip_time(Duration::from_nanos(999_999)),
            "999μs"
        );
        assert_eq!(
            format_round_trip_time(Duration::from_nanos(1_000_000)),
            "1ms"
        );
        ...
        assert_eq!(
            format_round_trip_time(Duration::from_secs(3600)),
            "3600.00s"
        );
    ...
    fn test_user_display_suitability() {
        ...
        for (duration, expected) in cases {
            ...
            assert!(
                result.len() <= 10,
                "Display format should be concise: '{}'",
                result
            );
            ...
            assert!(
                !result.contains(' '),
                "Display format should not contain spaces: '{}'",
                result
            );

            // Check it has appropriate unit suffix
            assert!(
                result.ends_with("μs") || result.ends_with("ms") || result.ends_with("s"),
                "Display format should have appropriate unit suffix: '{}'",
                result
            );
