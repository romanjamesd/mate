...
async fn test_interrupted_write_completion() {
    ...
    for (progress_percent, description) in progress_points {
        ...
        println!("Testing interrupted write at {}: byte position {}", description, interrupt_position);
        ...
        let result = framed_message.write_message(&mut interrupted_stream, &original_envelope).await;
        
        if result.is_err() {
            println!("✓ Write correctly failed due to WouldBlock at {} (expected behavior)", description);
            assert_eq!(interrupted_stream.interruption_count(), 1,
                       "Should have recorded exactly one interruption for {}", description);
        } else {
            ...
            println!("✓ Write unexpectedly succeeded for {} (implementation may have improved)", description);
        }
    }
